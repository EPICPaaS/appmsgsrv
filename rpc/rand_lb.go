...
func NewRandLB(clients map[string]*rpc.Client, addrs []string, service string, retry, ping time.Duration, check bool) (*RandLB, error) {
	length := len(clients)
	if length != len(addrs) {
		logger.Errorf("clients: %d, addrs: %d not equal", length, len(addrs))
		return nil, ErrRandLBLength
	}
	for _, addr := range addrs {
		if _, ok := clients[addr]; !ok {
			logger.Errorf("addr: \"%s\" not exist in clients map", addr)
			return nil, ErrRandLBAddr
		}
	}
	r := &RandLB{Clients: clients, addrs: addrs, length: length}
	if check && length > 0 {
		logger.Info("rpc ping start")
...
func (r *RandLB) Get() *rpc.Client {
	...
	logger.Tracef("rand hit rpc node: \"%s\"", addr)
...
func (r *RandLB) Stop() {
	...
	logger.Info("stop the randlb retry connect goroutine and ping goroutines")
}
	...
	for _, client := range r.Clients {
		if client != nil {
			if err := client.Close(); err != nil {
				logger.Errorf("client.Close() error(%v)", err)
			}
		}
	}
}
...
func (r *RandLB) ping(service string, retry, ping time.Duration) {
	...
	for _, addr := range r.addrs {
		...
		go func(addr string) {
			logger.Infof("\"%s\" rpc ping goroutine start", addr)
			ret := 0
			for {
				select {
				case <-r.exitCH:
					logger.Infof("\"%s\" rpc ping goroutine exit", addr)
					return
				default:
				}
				// get client for ping
				...
				if err := client.Call(method, 0, &ret); err != nil {
					...
					client.Close()
					retryCH <- addr
					logger.Errorf("client.Call(\"%s\", 0, &ret) error(%v), retry", method, err)
					time.Sleep(retry)
					continue
				}
				// if ok, sleep
				logger.Tracef("\"%s\": rpc ping ok", addr)
				time.Sleep(ping)
			}
		}(addr)
	}
	...
	go func() {
		...
		logger.Info("rpc retry connect goroutine start")
		for {
			select {
			case retryAddr = <-retryCH:
			case <-r.exitCH:
				logger.Info("rpc retry connect goroutine exit")
				return
			}
			rpcTmp, err := rpc.Dial("tcp", retryAddr)
			if err != nil {
				logger.Errorf("rpc.Dial(\"tcp\", %s) error(%s)", retryAddr, err)
				continue
			}
			logger.Infof("rpc.Dial(\"tcp\", %s) retry succeed", retryAddr)
			// copy-on-write
